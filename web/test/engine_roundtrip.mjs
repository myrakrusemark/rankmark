...
ok(c.ev.start.max_new === Math.ceil((c.ev.start.frame_bits / 0.5) * 1.3), `max_new from the carrier rate (got ${c.ev.start.max_new})`);
...
ok(fp !== fp3, "thread count changes the fingerprint");
ok(fingerprintDiff({ threads: 4, model: "a" }, { threads: 2, model: "a" }).join() === "threads", "fingerprintDiff names the differing field");
